...
const extractAllGreyscale = (jimpImage) => {
	...
	jimpImage
		.clone()
		.greyscale()
		.scan(0, 0, jimpImage.bitmap.width, jimpImage.bitmap.height, function (x, y, idx) {
			const r = this.bitmap.data[idx];
			const g = this.bitmap.data[idx + 1];
			const b = this.bitmap.data[idx + 2];
			allColors.add(rgbToHex([r, g, b]));
...
export const vectori = async (file) => {
	...
	const separatedColorSvgs = await traceImagesToSvgs(separatedColorImages, await colorPallet);
